...
13. `POST /v1/bookings` — book the past-dated event too, so it can be reviewed:
    ```json
    {"event_id": 2, "quantity": 1}
    ```
14. Wait ~5 seconds per booking (`PAYMENT_GATEWAY_DELAY_SECONDS`, simulating Razorpay's auto-capture processing time) — behind the scenes: order creation on `mock-razorpay` → simulated webhook → real `/v1/webhooks/razorpay` receiver → confirmation email logged by the worker. Then `GET /v1/bookings/{id}` for each — `status` has flipped to `"CONFIRMED"`.
15. `POST /v1/bookings/{id}/reviews` — using the booking for the **past** event (from step 13) once it's confirmed:
    ```json
...
16. `GET /v1/events/{id}/reviews` — using the past event's id — see the review returned.
