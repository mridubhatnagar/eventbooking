...
**4. `POST /v1/events`** — main event (organizer picks a date + 12-hour time, no ISO string/timezone — IST assumed)
```json
...
**5. `POST /v1/events`** — review-demo event. Past dates are rejected on create, so set `event_date`/`hour`/`minute` a few minutes ahead of when you're recording, then wait for it to pass before the review step:
```json
...
**13. `GET /v1/bookings/{BOOKING_ID_1}`** — no payload
→ `status` now `"CONFIRMED"`.

**14. `GET /v1/bookings/{BOOKING_ID_2}`** — no payload
→ `status` now `"CONFIRMED"`.

**15. `POST /v1/bookings/{BOOKING_ID_1}/reviews`** — future event, expect `400`
```json
{"rating": 5, "review_text": "Can't wait!"}
```
→ rejected: event hasn't happened yet.

**16. `POST /v1/bookings/{BOOKING_ID_2}/reviews`** — past event, expect `201`
```json
{"rating": 5, "review_text": "Great show!"}
```

**17. `GET /v1/events/{EVENT_ID_2}/reviews`** — no payload
