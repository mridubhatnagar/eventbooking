...
## Database Migrations
Schema changes go through **Alembic** via **Flask-Migrate** (`migrate = Migrate()` in `app/extensions.py`, `migrate.init_app(app, db)` in `create_app()`) — the app no longer calls `db.create_all()` at startup. All commands run inside Docker (no local Python env): after changing a model, generate a migration with `docker compose run --rm --volume "$(pwd)/migrations:/app/migrations" app flask --app run db migrate -m "description"` (the volume mount is what gets the generated file out of the container and onto the host so it can be committed), review it under `migrations/versions/`, then apply with `docker compose up --build` — the `app` service's command already runs `flask --app run db upgrade` before starting `gunicorn`, so it applies automatically on every deploy. Tests are the one exception: the `app` fixture (`tests/conftest.py`) calls `db.create_all()` directly against the throwaway per-test DB — no migration history needed there.
