...
@api.validate(json=CreateBookingRequest, tags=["bookings"])
def create_booking():
...
@bp.get("")
@role_required(Role.CUSTOMER)
@api.validate(tags=["bookings"])
def list_bookings():
    user_id = int(get_jwt_identity())
    bookings = booking_service.list_bookings(user_id)
    return jsonify([_serialize(b) for b in bookings]), 200
...
def get_booking(booking_id):
    ...
    return jsonify(_serialize(booking)), 200
